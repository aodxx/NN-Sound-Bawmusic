...
- ลูกค้าเลือกอุปกรณ์จริงพร้อมจำนวนจากหน้า LIFF และระบบตรวจสอบสต็อก/คิวซ้ำฝั่ง Apps Script
- คำแนะนำการระบุสถานที่: ใช้ GPS เมื่ออยู่หน้างาน หรือปักหมุด/พิมพ์ที่อยู่เมื่อยังไม่อยู่หน้างาน
- หน้าแอดมินมีตัวเลขและไฟกระพริบแจ้งคำขอจองสถานะ pending
- เติมเบอร์โทรจาก LINE ได้เมื่อช่องทางได้รับอนุญาต LINE Profile+ scope: phone; หากไม่ได้รับอนุญาตยังกรอกเองได้
...
- กรอกเบอร์โทร
- เลือกจังหวัด
- เลือกวันที่จัดงานจากปฏิทินวันว่าง
- กดจากตารางงานของ Customer Portal เพื่อเปิดหน้าจองพร้อมเติมวันที่
- เลือกเวลา
- เลือกประเภทงาน
- ดูภาพอุปกรณ์จริงจากระบบ
- แนบตำแหน่งปัจจุบันด้วย GPS หรือวางลิงก์ Google Maps เอง
...
### ระยะ 2A: ลดขั้นตอนก่อนส่งคำขอจอง

- Customer Portal ใช้ <code>listPublicSchedule</code> เพื่อแสดงวันที่มีงานยืนยันแล้ว
- วันที่ที่ยังไม่ถูกระบุว่ามีงานยืนยันแล้วจะแสดงเป็นวันที่เลือกจองได้
- เมื่อลูกค้าแตะวันที่ว่าง ระบบจะเปิด LIFF จองคิวพร้อมพารามิเตอร์ <code>?date=YYYY-MM-DD</code>
- หน้า LIFF จองคิวอ่านวันที่ดังกล่าวและเติมลงฟอร์มให้อัตโนมัติ
- หน้า LIFF มีปฏิทินวันว่างในเดือนนั้นและปุ่มตรวจข้อมูลใหม่
- สถานที่ยังกรอกเป็นข้อความได้ตามปกติ และเลือกแนบ GPS หรือลิงก์ Google Maps ในช่อง <code>mapLink</code> ได้
- การอ่าน GPS ทำเมื่อผู้ใช้กดปุ่มเท่านั้น ไม่ติดตามตำแหน่งต่อเนื่อง และต้องได้รับอนุญาตจากเบราว์เซอร์
...
### PR #41: ทำให้การจองจาก LINE ใช้งานได้ครบขึ้น

- หน้า LIFF ให้ลูกค้าแตะเลือกอุปกรณ์จริงและกำหนดจำนวน
- Backend ตรวจว่าอุปกรณ์มีอยู่ในคลังและจำนวนไม่เกินสต็อก ก่อนตรวจชนคิวและบันทึก
- ข้อมูลอุปกรณ์ที่บันทึกใน Booking มี id, name, unit และ qty เพื่อให้หน้า Admin แสดงต่อได้
- เพิ่มคำอธิบายสถานที่แบบสองกรณี: อยู่หน้างานให้ใช้ GPS; ยังไม่อยู่หน้างานให้ปักหมุด Google Maps หรือกรอกชื่อ/ที่อยู่
- เพิ่มตัวเลขแจ้งเตือนคำขอ pending ที่ส่วนหัวและเมนูการจองของ Admin
- เพิ่มไฟกระพริบเฉพาะจุดที่มีคำขอใหม่ และไฮไลต์รายการ pending เพื่อให้เห็นงานที่ต้องตรวจสอบทันที
- ระบบตรวจสอบคำขอใหม่ทุก 45 วินาที และตรวจใหม่เมื่อกลับมาเปิดแท็บ
- เบอร์โทรจาก LINE จะถูกเติมอัตโนมัติเฉพาะเมื่อช่องทางได้รับข้อมูลจาก LINE Profile+; ถ้าไม่ได้รับสิทธิ์ ระบบจะให้ลูกค้ากรอกเองโดยไม่ทำให้การจองล้มเหลว
- มีการปิด animation เมื่ออุปกรณ์เปิดโหมดลดการเคลื่อนไหว
...
5. ถ้าช่องทางได้รับอนุญาต LINE Profile+ ให้เปิด <code>phone</code> เพื่อเติมเบอร์โทรอัตโนมัติ; ถ้าไม่มีสิทธิ์นี้ ลูกค้าจะกรอกเบอร์โทรเอง
6. เปิดลิงก์ผ่านแอป LINE เพื่อทดสอบ
