...
5. หากต้องการให้ระบบเติมเบอร์โทรอัตโนมัติ ให้ตรวจสอบว่า LINE Login Channel ได้รับอนุญาต LINE Profile+ และเปิด scope `phone` ใน LINE Developers Console หากช่องทางยังไม่ได้รับสิทธิ์นี้ LINE จะไม่ส่งเบอร์โทรมา และลูกค้าจะกรอกเองได้
...
### การเลือกอุปกรณ์และสถานที่จาก LIFF

- ลูกค้าสามารถแตะการ์ดเพื่อเลือกอุปกรณ์และกำหนดจำนวนได้
- ระบบตรวจสอบจำนวนพร้อมใช้งานและรายการจองที่ชนกันทั้งก่อนส่งและขณะบันทึกจริง
- ถ้าอยู่ที่สถานที่จัดงาน ให้กดใช้ตำแหน่งปัจจุบัน
- ถ้ายังไม่ได้อยู่ที่สถานที่ ให้พิมพ์ชื่อ/ที่อยู่ หรือวางลิงก์ Google Maps จากการปักหมุด

### สถานะการจองจาก LIFF
การจองที่ลูกค้าส่งผ่าน LIFF จะถูกบันทึกด้วยสถานะ **"รอยืนยัน" (pending)** เสมอ — หน้า Admin จะแสดงจำนวนและไฟกระพริบเมื่อมีคำขอใหม่ แอดมินต้องเปิดรายการจองนั้น กรอกราคา/แพ็คเกจ แล้วเปลี่ยนสถานะเป็น "ยืนยันแล้ว" เอง (ยังไม่มีการแจ้งเตือนอัตโนมัติเข้า LINE — เป็นเฟสถัดไปที่ต้องใช้ Channel Access Token)
...
listPublicEquipment      — ส่งรายการอุปกรณ์พร้อมจำนวนที่เลือกได้และรูปภาพให้หน้า LIFF
uploadEquipmentImage     — อัปโหลดรูปอุปกรณ์ไปยัง Google Drive (ต้องมี session แอดมิน)
createPayment / updatePayment / deletePayment / listPayments / getPayment / getPaymentSummary
listAuditLog

### การทำงานของ Payments

- การจองใหม่ที่มียอดชำระเริ่มต้นจะสร้างแถวในชีต Payments ให้อัตโนมัติ
- การเพิ่ม/แก้ไข/ลบรายการชำระเงินจะคำนวณ <code>deposit</code> และ <code>remaining</code> ใน Bookings ใหม่
- ห้ามแก้ยอดชำระโดยตรงใน Bookings เมื่อมีรายการ Payments แล้ว ให้แก้ผ่าน API/หน้าแอปเท่านั้น
- ชนิดรายการที่รองรับ: <code>deposit</code>, <code>partial</code>, <code>final</code>, <code>refund</code>
- ช่องทางที่รองรับ: เงินสด, โอนธนาคาร, พร้อมเพย์, บัตร และอื่นๆ
...
### ระบบสำรองข้อมูลทั้ง Spreadsheet

หลัง Deploy Backend เฟส 2 แล้ว ให้เปิดหน้า **ตั้งค่า → ป้องกันข้อมูลสูญหาย** และทำตามลำดับ:

1. กด **ติดตั้งสำรองรายสัปดาห์** เพื่อสร้าง Trigger ทุกวันจันทร์ช่วงประมาณ 03:00–04:00 น. ตามเวลา Asia/Bangkok
2. กด **สำรองตอนนี้** และเปิดไฟล์ที่ระบบสร้าง เพื่อตรวจว่าทุกชีตและข้อมูลครบ
3. กด **ดาวน์โหลดข้อมูลทั้งระบบ** และเปิดไฟล์ `.xlsx` เพื่อตรวจว่าใช้งานได้
4. เปิด Google Sheets → ไฟล์ → ประวัติเวอร์ชัน → ดูประวัติเวอร์ชัน แล้วจึงกด **ยืนยัน Version History**

ระบบจะสร้างโฟลเดอร์ `Bawmusic Backups` ให้อัตโนมัติ และบันทึก ID เป็น Script Property `DRIVE_BACKUP_FOLDER_ID` หากต้องการใช้โฟลเดอร์ที่เตรียมไว้เอง ให้ตั้งค่านี้ก่อนกดปุ่มครั้งแรก โฟลเดอร์สำรองควรเป็น `Restricted` และแชร์เฉพาะผู้ดูแลที่จำเป็น
