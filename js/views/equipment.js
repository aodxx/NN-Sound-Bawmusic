  ...
  try {
    container.innerHTML = `
      ...
      ${EQUIPMENT_CATEGORIES.map(cat => {
        ...
        return `
          <div class="mb-4">
            <h3 class="text-sm font-semibold text-gold mb-2"><i class="fa-solid ${CATEGORY_ICONS[cat] || 'fa-box'} mr-1.5"></i>${CATEGORY_LABELS[cat] || cat}</h3>
            <div class="space-y-2">
      ...
      ${(() => {
        ...
        return `
          <div class="mb-4">
            <h3 class="text-sm font-semibold text-gold mb-2"><i class="fa-solid fa-box mr-1.5"></i>อื่นๆ</h3>
  ...
  return `
    <div class="bg-navy-light rounded-xl p-3 border border-gold/10 flex items-center justify-between" onclick="window.__openEquipmentForm('${e.id}')">
      <div>
        ...
        ${e.remarks ? `<p class="text-sm text-gray-500">${e.remarks}</p>` : ''}
      </div>
